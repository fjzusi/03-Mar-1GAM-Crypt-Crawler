...
function LevelTiles:initialize()
	...
	-- Floor 5
	self.tileQuads[8] = love.graphics.newQuad(
		8 * (TILE_SIZE + 1),
		0 * TILE_SIZE,
		TILE_SIZE + 1,
		TILE_SIZE + 1,
	...
	-- Wall 5
	self.tileQuads[9] = love.graphics.newQuad(
		9 * (TILE_SIZE + 1),
		0 * TILE_SIZE,
		TILE_SIZE + 1,
		TILE_SIZE + 1,
...
function LevelTiles:addTile(x, y, value)
	...
	if string.find(value, "T8") ~= nil then
		self.tilesetBatch:addq(self.tileQuads[8], x, y)
	end
